...
public class ZePedroEntity extends Guardian {
  ...
  protected void registerGoals() {
    ...
    this.goalSelector.getAvailableGoals()
        .removeIf(goal -> goal.getGoal().getClass().getName().contains("GuardianAttackGoal"));

    this.targetSelector.getAvailableGoals()
        .removeIf(goal -> goal.getGoal().getClass().getName().contains("NearestAttackableTargetGoal"));
  ...
  private static class ZePedroTargetGoal extends TargetGoal {
    ...
    public boolean canContinueToUse() {
      ...
      if (!this.target.isAlive())
        return false;
      if (this.zePedro.distanceToSqr(this.target) > 16.0D * 16.0D)
        return false;
